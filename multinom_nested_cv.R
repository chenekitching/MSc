  ...
  foreach(i = 1:length(preds)) %dofuture% {
    preds[[i]] %>% 
      mutate(pred_class = case_when(
        .pred_P_LP >= .pred_B_LB & .pred_P_LP >= .pred_VUS ~ "P_LP",
        .pred_B_LB >= .pred_P_LP & .pred_B_LB >= .pred_VUS ~ "B_LB",
        .pred_VUS >= .pred_P_LP & .pred_VUS >= .pred_B_LB ~ "VUS"
      )) %>% 
...
names(rs_hard_preds) <- names(mod_list)
...
get_cv_metrics <- function(preds){
   preds$pred_class <- factor(preds$pred_class, levels = c("P_LP", "B_LB", "VUS"))
  preds %>% 
    group_by(resample) %>%
